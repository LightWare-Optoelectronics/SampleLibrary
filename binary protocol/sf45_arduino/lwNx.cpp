...
bool lwnxHandleManagedCmd(lwSerialPort* Ser, uint8_t CommandId, uint8_t* Response, uint32_t ResponseSize, bool Write, uint8_t* WriteData, uint32_t WriteSize) {
	int32_t attempts = PACKET_RETRIES;

	while (attempts--) {
		lwnxSendPacketBytes(Ser, CommandId, Write, WriteData, WriteSize);

		lwResponsePacket response;
		
		if (lwnxRecvPacket(Ser, CommandId, &response, PACKET_TIMEOUT)) {
